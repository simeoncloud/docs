...
"@
    ...
    function Install-SimeonTenantAzureDevOps {
        ...
        Wait-EnterKey "Connecting to Azure DevOps - if prompted, $loginInstructions"
        ...
        if (!(Test-SimeonAzureDevOpsAccessToken -Organization $Organization -Project $Project -Token $token)) {
            Wait-EnterKey "Retrying Azure DevOps login - $loginInstructions"
            $token = Get-SimeonAzureDevOpsAccessToken -LaunchBrowser -PromptForLogin
        }
    
        if (!(Test-SimeonAzureDevOpsAccessToken -Organization $Organization -Project $Project -Token $token)) {
            Wait-EnterKey "Retrying Azure DevOps login - please copy the below url, paste into an incognito/private browser window and $loginInstructions"
